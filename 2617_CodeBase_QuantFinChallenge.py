"""Quadrant v5.3 Strategy — Research Pipeline (Two-Parquet Support)

This script orchestrates the end-to-end pipeline used in the hackathon:

  1) Read market data from Parquet (Period 1 required, Period 2 optional).
  2) Build per-stock datasets and sector indices (MCAP and Smart Beta).
  3) Run the Quadrant v5.3 strategy on the generated folders.
  4) Produce a performance report versus Nifty 500 (via yfinance).

Two-period mode
---------------
- Period 1 (e.g., 2010–2020): full backtest.
- Period 2 (e.g., 2020–2025): optional. When enabled, the strategy is warmed up on
  Period 1 + Period 2 history, but trades are only executed on/after the start of
  Period 2. Reporting/CAGR is computed strictly over the Period 2 dataset span.

User-editable parameters are grouped under **USER CONFIGURATION** below.

Requirements
------------
Python 3.10+; pandas, numpy, pyarrow, scipy, matplotlib, yfinance.
...
# ============================================================================
# Quadrant v5.3 Strategy — Merged Runner
# ============================================================================
...
# =============================================================================
# USER CONFIGURATION (edit these parameters only)
# =============================================================================

# --- Parquet inputs (two-period support) ---
# Period 1 (required; e.g., 2010–2020)
PARQUET_FILE_P1 = r"nse_prices_complete 1.parquet"

# Period 2 (optional; e.g., 2020–2025)
# Set USE_SECOND_DATASET = True and provide PARQUET_FILE_P2 when available.
USE_SECOND_DATASET = False
PARQUET_FILE_P2 = r""

# --- Output root ---
# Each period run will write into a separate subfolder under OUTPUT_ROOT.
OUTPUT_ROOT = os.getcwd()

# --- Index base value used when constructing indices ---
BASE_VALUE = 100

# --- Benchmark configuration (Nifty 500) ---
# The script tries these tickers (in order) until one succeeds.
BENCHMARK_TICKERS = ["^CRSLDX", "NIFTY500.NS", "^NIFTY500", "^NSE500"]
TRADING_DAYS = 252

# --- Sector mapping used by the folder/index builder ---
GICS_MAP = {
    "Energy": 10,
    "Materials": 15,
    "Industrials": 20,
...
# =============================================================================
# INTERNAL RUNTIME CONTROLS (set automatically; do not edit)
# =============================================================================

# Will be overwritten per run:
OUTPUT_BASE = OUTPUT_ROOT

# Used by performance reporting (CAGR window):
DATASET_FILE_START = None
DATASET_FILE_END = None

# Trading gate: if set, no trades are executed before this date (warm-up allowed).
TRADE_START_DATE = None

# Reporting controls set by the merged runner:
PERF_OUTPUT_PREFIX = None
PERF_PERIODS = None
...
def _perf_cagr(prices: pd.Series, start_date=None, end_date=None) -> float:
    prices = prices.dropna()
    if len(prices) < 2:
        return float('nan')

    # Priority:
    # 1) explicit start_date/end_date passed in
    # 2) parquet dataset span (DATASET_FILE_START/END) if available
    # 3) fallback: prices index span (old behavior)
    s = start_date
    e = end_date

    if s is None or e is None:
        ds = globals().get("DATASET_FILE_START", None)
        de = globals().get("DATASET_FILE_END", None)
        if s is None:
            s = ds
        if e is None:
            e = de
...
def _read_parquet_to_df(parquet_path: str) -> pd.DataFrame:
    """Read a parquet file into a pandas DataFrame."""
    print(f"Loading parquet file: {parquet_path}")
    table = pq.read_table(parquet_path)
    return table.to_pandas()

def _infer_tradedate_range(df: pd.DataFrame):
    """Infer [start,end] from df['tradedate']."""
    if 'tradedate' not in df.columns:
        return None, None
    td = pd.to_datetime(df['tradedate'], errors='coerce')
    td = td.dropna()
    if td.empty:
        return None, None
    return td.min(), td.max()

def folder_build_from_df(df: pd.DataFrame, output_base: str, report_start=None, report_end=None):
    """
    Build dataset/sector folders + indices from a DataFrame.

    - output_base: where 'dataset/', 'mcap_index/', 'smart_beta_index/' will be created.
    - report_start/report_end: the span you want reporting/CAGR to use (can differ from df span).
    """
    global OUTPUT_BASE, DATASET_FILE_START, DATASET_FILE_END
    OUTPUT_BASE = output_base
    os.makedirs(OUTPUT_BASE, exist_ok=True)

    ds_start, ds_end = _infer_tradedate_range(df)

    # Reporting span (used by _perf_cagr)
    DATASET_FILE_START = pd.to_datetime(report_start) if report_start is not None else ds_start
    DATASET_FILE_END   = pd.to_datetime(report_end)   if report_end is not None else ds_end

    if DATASET_FILE_START is not None and DATASET_FILE_END is not None:
        print(f"[DATASET RANGE] {DATASET_FILE_START.date()} -> {DATASET_FILE_END.date()}")
    else:
        print("[DATASET RANGE] Could not infer tradedate range (missing/invalid 'tradedate').")
...
def main():
    """
    Backward-compatible single-parquet run for folder/index building.
    (The merged runner below uses folder_build_from_df(...) directly.)
    """
    df = _read_parquet_to_df(PARQUET_FILE_P1)
    folder_build_from_df(df, OUTPUT_BASE)
...
KELLY_MULTIPLIER = 0.70         # Half-Kelly fix 0.80
...
def main():
    ...
    for i in range(start_idx, len(all_dates)):
        ...
        # Use earlier data for indicators, but do NOT execute trades before TRADE_START_DATE.
        # ------------------------------------------------
        if (TRADE_START_DATE is not None) and (pd.to_datetime(date) < pd.to_datetime(TRADE_START_DATE)):
            pending_rebal = None
    ...
    try:
        _perf_run_reporting(
            eq,
            output_prefix=(PERF_OUTPUT_PREFIX if PERF_OUTPUT_PREFIX is not None else "quadrant_v5"),
            trading_days=TRADING_DAYS,
            benchmark_tickers=BENCHMARK_TICKERS,
            periods=(PERF_PERIODS if PERF_PERIODS is not None else [
                ("2010-2020", "2010-01-01", "2020-12-31"),
            ]),
        )
...
# NOTE:
# The embedded strategy module includes a standalone runner guarded by __main__.
# In this merged script, execution is orchestrated by merged_main() at the bottom.
# The strategy runner is therefore intentionally not invoked here.
#
...
def _run_period(label: str,
    ...
    Run ONE full pipeline:
      1) build folders/indices from df_build into output_base
      2) run strategy using those folders
      3) run performance reporting for [report_start, report_end]

    If trade_start is set, no trades are executed before that date (warm-up allowed).
    """
    global DATASET_FOLDER, MCAP_FOLDER, SMARTBETA_FOLDER
    global TRADE_START_DATE, PERF_OUTPUT_PREFIX, PERF_PERIODS
    ...
    TRADE_START_DATE = pd.to_datetime(trade_start) if trade_start is not None else None

    # Performance-report window (auto)
    PERF_OUTPUT_PREFIX = label.replace(" ", "_").replace("/", "_").replace("\\", "_")
    PERF_PERIODS = [(
        label,
        pd.to_datetime(report_start).date().isoformat(),
        pd.to_datetime(report_end).date().isoformat()
    )]

    # Keep all outputs (PNGs/CSVs) inside this run folder
    old_cwd = os.getcwd()
...
def merged_main():
    # ---------------------------
    # PERIOD 1 (e.g., 2010–2020)
    # ---------------------------
    df1 = _read_parquet_to_df(PARQUET_FILE_P1)
    p1_s, p1_e = _infer_tradedate_range(df1)
    if p1_s is None or p1_e is None:
        raise RuntimeError("Could not infer 'tradedate' range for PARQUET_FILE_P1.")

    label1 = f"{p1_s.year}-{p1_e.year}"
    out1 = os.path.join(OUTPUT_ROOT, label1.replace("-", "_"))
    _run_period(
        label=label1,
        df_build=df1,
        output_base=out1,
    ...
    # ---------------------------
    # PERIOD 2 (e.g., 2020–2025) — optional
    # Warm-up: uses PERIOD 1 data, but trades only execute from PERIOD 2 start.
    # ---------------------------
    if USE_SECOND_DATASET:
        try:
            df2 = _read_parquet_to_df(PARQUET_FILE_P2)
        except Exception as e:
            print(f"[PERIOD 2] Skipping: could not load PARQUET_FILE_P2: {e}")
            return

        p2_s, p2_e = _infer_tradedate_range(df2)
        if p2_s is None or p2_e is None:
            print("[PERIOD 2] Skipping: could not infer 'tradedate' range for PARQUET_FILE_P2.")
            return

        # Combine for warm-up computations (no pre-period trades due to TRADE_START_DATE)
        try:
            df_comb = pd.concat([df1, df2], ignore_index=True)
        except Exception:
            df_comb = df2.copy()

        if 'tradedate' in df_comb.columns:
            df_comb['tradedate'] = pd.to_datetime(df_comb['tradedate'], errors='coerce')
            df_comb = df_comb.sort_values('tradedate').reset_index(drop=True)

        label2 = f"{p2_s.year}-{p2_e.year}"
        out2 = os.path.join(OUTPUT_ROOT, label2.replace("-", "_"))
        _run_period(
            label=label2,
            df_build=df_comb,
            output_base=out2,
            trade_start=p2_s,       # Trades start at the beginning of Period 2
            report_start=p2_s,      # Reporting/CAGR window = Period 2 span
            report_end=p2_e
        )
